...
presetButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    activeCase = button.dataset.case || activeCase;
    playing = true;
    playPauseButton.textContent = "暂停";
    resetBattle();
  });
...
const mockCommandInput = document.getElementById("mockCommandInput");
const runMockCommandButton = document.getElementById("runMockCommand");

function interpretMock(command) {
  const lower = command.toLowerCase();
  if (/集火|focus|残血|weakest/.test(lower)) return "good_focus";
  if (/冲|charge|rush|全员上|猛攻/.test(lower)) return "bad_charge";
  if (/稳|防守|hold|保护.*后排|hold_all/.test(lower)) return "hold_all";
  if (/趴|prone|别打|stop firing|cower/.test(lower)) return "cower_all";
  if (/原地|不动|wait|等/.test(lower)) return "hold_all";
  return "hesitate";
}

function runMockCommand() {
  const command = mockCommandInput.value.trim();
  if (!command) return;
  const policy = interpretMock(command);
  cases["mock_command"] = {
    command: command,
    policy: policy,
    result: "(mock — see Python simulator for statistics)",
    note: `Mock: "` + command + `" → ` + policy + ` (keyword-based local interpreter)`,
  };
  activeCase = "mock_command";
  playing = true;
  playPauseButton.textContent = "暂停";
  commandText.textContent = command;
  policyText.textContent = policy + " [mock]";
  resultText.textContent = "(mock — no statistics)";
  presetButtons.forEach((item) => item.classList.remove("is-active"));
  resetBattle();
}

runMockCommandButton.addEventListener("click", runMockCommand);
mockCommandInput.addEventListener("keydown", (event) => {
  if (event.key === "Enter") runMockCommand();
});
