...
from game.mirror_map_sim import run_batch, simulate_mirror_battle


def print_single(seed: int, jitter: int) -> None:
    result = simulate_mirror_battle(seed=seed, jitter=jitter)
...
def print_batch(runs: int, seed: int, jitter: int) -> None:
    stats = run_batch(runs=runs, seed=seed, jitter=jitter)
    print("mode=mirror-3v3-map")
...
def main() -> None:
    ...
    if args.runs <= 1:
        print_single(args.seed, args.jitter)
    else:
        print_batch(args.runs, args.seed, args.jitter)
