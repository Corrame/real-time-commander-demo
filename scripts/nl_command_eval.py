...
def clean_policy_result(data: dict[str, Any], used_llm: bool) -> tuple[str, str, bool]:
    ...
    if policy not in POLICIES:
        # Contract failure: a missing or out-of-vocabulary policy must surface as
        # an error, not be silently coerced into "hesitate". Otherwise a broken
        # interpreter would masquerade as a legitimate "irrelevant chat" result
        # and pollute the win-rate statistics.
        raise LLMError(f"LLM returned invalid policy: {raw_policy!r}")
